...
fall_delay = 30
fall_counter = 0
music_started = False  # controle para iniciar a música uma única vez
...
def update():
    global fall_counter, current_piece, game_over, start_screen, music_started

    # Iniciar música de fundo uma única vez
    if not music_started:
        music.set_volume(0.5)
        music.play('background')  # Certifique-se que background.ogg está na pasta "music"
        music_started = True
...
def draw():
    ...
    if start_screen:
        screen.draw.text("TETRIS CLONE", center=(WIDTH//2, HEIGHT//2 - 40), fontsize=40, color="white")
        screen.draw.text("Pressione ENTER para começar", center=(WIDTH//2, HEIGHT//2 + 10), fontsize=20, color="gray")
        return
...
def on_key_down(key):
    global current_piece, start_screen, game_over, score, grid
